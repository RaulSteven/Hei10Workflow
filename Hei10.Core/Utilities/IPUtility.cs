...
namespace Hei10.Core.Utilities
{
    public class IPUtility
    {
        ...
        {
            return GetIPAddress(GetIP());
        }

        public string GetIPAndCity()
        {
            var ip = GetIP();
            return string.Format("{0}({1})", ip, GetIPAddress(ip));
        }

        public string GetIPAddress(string ip)
        {
            string path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"App_Data\qqwry.dat");
            return GetIPAddress(path, ip);
        }

        public string GetIPAddress(string path, string ip)
        {
            dataPath = path;
            this.ip = ip;
            QQwry();
